...
export default function ApprovalsPage() {
  ...
  const getFilteredBudgets = () => {
    ...
    switch (user.role) {
      case '管理部長':
        filtered = filtered.filter(b => b.status === '承認待ち（管理部長）');
        break;
      case '常務':
        filtered = filtered.filter(b => b.status === '承認待ち（常務）');
        break;
      case '社長':
        filtered = filtered.filter(b => b.status === '承認待ち（社長）');
        break;
      default:
